#!/bin/ash
## LEGO CA Certificate Request and Node-RED launch
...
if [[ "$1" == "firstStart" ]]; then
    echo -n $(date) - Node-RED First Start...
    crond -b -s /etc/cron.d -L /dev/stdout
    sudo -u node-red node-red -p 1880 -s /data/settings.js -u /data
else
    echo -n $(date) - Restarting Node-RED...
    sudo -u node-red node-red-restart
fi
